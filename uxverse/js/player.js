/* Player: boots the film, scales the 1920x1080 stage, drives the clock and the soundtrack. */
(() => {
  ...
  const timeEl = $('time'), scrub = $('scrub'), fill = $('fill'), head = $('head'), chapterEl = $('chapter'), status = $('status');
  ...
  async function fontsReady() {
    const faces = ['400 100px Anton', '400 100px Bangers', '700 100px Kalam', '400 100px Kalam', '700 100px "Space Mono"', '400 100px "Space Mono"'];
    try { await Promise.all(faces.map((x) => document.fonts.load(x))); } catch (e) { /* fall back */ }
  ...
  ready.then(() => {
    const { DUR, CHAPTERS, render } = F;
    let wav = null;
    window.__film = {
      DUR,
      render: (t) => render(t),
      async audio() { const buf = await Score.render(F.CUES, DUR, 48000); wav = Score.toWav(buf); return wav.length; },
      audioChunk(i, size) { const part = wav.subarray(i, i + size); let s = ''; for (let k = 0; k < part.length; k += 0x8000) s += String.fromCharCode.apply(null, part.subarray(k, k + 0x8000)); return btoa(s); },
    };
    if (capture) { render(0); window.__filmReady = true; return; }

    CHAPTERS.forEach((c) => {
      if (c.t <= 0) return;
      const tk = document.createElement('div'); tk.className = 'tick'; tk.style.left = (c.t / DUR) * 100 + '%'; tk.title = c.name; scrub.appendChild(tk);
    });
    ...
    async function loadSoundtrack() {
      try {
        ...
        return Score.render(F.CUES, DUR, 44100, (p) => { status.textContent = `Scoring soundtrack… ${Math.round(p * 100)}%`; });
      }
    }
    loadSoundtrack().then((b) => { buffer = b; status.textContent = ''; if (playing) startAudio(now()); })
    ...
    function ensureCtx() {
      ...
      if (!AC) return;
      actx = new AC(); gainNode = actx.createGain(); gainNode.gain.value = muted ? 0 : 1; gainNode.connect(actx.destination);
    }
    function startAudio(t) {
      stopAudio();
      if (!actx || !buffer || t >= DUR) return;
      src = actx.createBufferSource(); src.buffer = buffer; src.connect(gainNode);
      src.start(actx.currentTime + 0.03, t);
      clock = { ctx: true, t0: actx.currentTime + 0.03, off: t };
    }
    function stopAudio() { if (src) { try { src.stop(); } catch (e) { /* stopped */ } src.disconnect(); src = null; } }

    let playing = false, cur = POSTER_T, clock = null, raf = 0;
    function now() {
    ...
    function play() {
      ...
      poster.hidden = true; playing = true;
    ...
    function pause() {
      if (!playing) return;
      cur = Math.min(DUR, now()); playing = false; stopAudio(); cancelAnimationFrame(raf);
      playBtn.innerHTML = ICON_PLAY; playBtn.setAttribute('aria-label', 'Play');
    }
    function seek(t) {
      t = Math.max(0, Math.min(DUR, t));
      poster.hidden = true;
      if (playing) { cur = t; clock = { ctx: false, p0: performance.now(), off: t }; startAudio(t); } else { cur = t; draw(t); }
    }
    poster.addEventListener('click', () => { cur = 0; play(); });
    playBtn.addEventListener('click', () => { if (playing) return pause(); if (!poster.hidden) cur = 0; play(); });
    muteBtn.addEventListener('click', () => { muted = !muted; if (gainNode) gainNode.gain.value = muted ? 0 : 1; muteBtn.textContent = muted ? 'Sound off' : 'Sound on'; });
    fsBtn.addEventListener('click', async () => { try { if (document.fullscreenElement) await document.exitFullscreen(); else await screen.requestFullscreen(); } catch (e) { /* not allowed */ } });
    let dragging = false;
